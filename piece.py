...
class Piece:
    ...
    def deplacement_autorise(self, case_souhaitee):
        ...
        if self.name == "pawn":
            ...
            def avancer_pion(
                ...
                if pos == self.position + Position(0, 1) and self.case_autorisee(
                    case_souhaitee
                ):
                    self.memory = "Moved"
                    return True
            ...
            def avancer_2_pion(case_souhaitee):
                pos = Position(*case_souhaitee.position)
                if (
                    pos == self.position + Position(0, 2)
                    and self.case_autorisee(case_souhaitee)
                    and self.case_autorisee(self.position + Position(0, 1))
                    and self.memory == "Didn't move yet"
                ):
            ...
            def manger_pion(case_souhaitee):
                ...
                pos = Position(*case_souhaitee.position)
                return pos in {haut_droit, haut_gauche} and self.case_autorisee(
                    case_souhaitee
                )
    ...
    def case_autorisee(self, case_souhaitee):  # case_souhaitee doit être de classe Case
        ...
        if p_c[i][j] == 0:
            return True
        return dico_n_c[p_c[i][j]] != self.color

    def deplacement(self, case_souhaitee):
        if self.deplacement_autorise(case_souhaitee) and self.case_autorisee(
            case_souhaitee
        ):
